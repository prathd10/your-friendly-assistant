...
const CoverSlide = ({ slide }: { slide: Slide }) => (
  <div className="h-full w-full flex flex-col justify-between p-10 relative overflow-hidden bg-[#0a0a0f]" style={{ width: '1280px', height: '720px' }}>
      ...
      <div className="space-y-4">
        ...
        {slide.subtitle && (
          <div className="flex items-center gap-3">
            ...
            <p className="text-xs text-white/40 font-semibold tracking-widest uppercase">{slide.subtitle}</p>
          </div>
        )}
      </div>
    </div>
    ...
    <div className="z-10 flex items-center justify-between">
      ...
      <span className="text-[8px] uppercase tracking-[0.4em] text-white/20 font-medium pl-6">Confidential • Powered by AI</span>
    </div>
  </div>
);

const ContentSlide = ({ slide }: { slide: Slide }) => (
  <div className="h-full w-full flex flex-col p-12 gap-8 relative overflow-hidden bg-[#0a0a0f]" style={{ width: '1280px', height: '720px' }}>
    ...
    <ul className="space-y-4 flex-1 flex flex-col justify-center z-10">
...
const formatMetricValue = (val: any): string => {
  ...
  if (typeof val === 'object') {
    if (val.gender && val.ageRange) {
      const gender = Array.isArray(val.gender) ? val.gender[0] : val.gender;
      const age = Array.isArray(val.ageRange) ? `${val.ageRange[0]}-${val.ageRange[1]}` : val.ageRange;
      return `${gender} • ${age}`;
    }
    return 'Premium Segment';
  }

  // Handle JSON strings (for old data)
  if (typeof val === 'string' && (val.startsWith('{') || val.startsWith('['))) {
    try {
      const parsed = JSON.parse(val);
      if (parsed.gender || parsed.ageRange) {
        const gender = Array.isArray(parsed.gender) ? parsed.gender[0] : (parsed.gender || 'All');
        const age = Array.isArray(parsed.ageRange) ? `${parsed.ageRange[0]}-${parsed.ageRange[1]}` : (parsed.ageRange || '18-40');
        return `${gender} • ${age}`;
      }
    } catch (e) {
      // Not JSON, continue
...
const MetricsSlide = ({ slide }: { slide: Slide }) => (
  <div className="h-full w-full flex flex-col p-10 gap-6 relative overflow-hidden bg-[#0a0a0f]" style={{ width: '1280px', height: '720px' }}>
    {/* Clean background for capture reliability */}
    <div className="absolute inset-0 bg-[#0a0a0f]" />
    <div className="absolute -bottom-20 -left-20 w-[600px] h-[600px] opacity-[0.07] rounded-full blur-[120px] bg-primary" />
    
    <div className="space-y-2 z-10">
      <div className="h-1.5 w-12 rounded-full bg-primary" />
      <h2 className="text-4xl md:text-5xl font-black tracking-tighter text-white uppercase italic leading-none">{slide.title}</h2>
    </div>
    
    {/* Metrics grid - Compacted to prevent overflow */}
    {slide.metrics && slide.metrics.length > 0 && (
      <div className="grid grid-cols-2 gap-6 flex-1 items-center z-10 py-2">
        {slide.metrics.map((m, idx) => (
          <div key={idx} className="relative">
            <div className="relative rounded-[2rem] border border-white/10 bg-white/[0.03] p-8 flex flex-col gap-2 backdrop-blur-xl">
    ...
    {/* Supporting content - Premium list - Compacted */}
    {slide.content && slide.content.length > 0 && (
      <div className="grid grid-cols-2 gap-10 z-10 border-t border-white/10 pt-6 mt-auto">
        {slide.content.map((bullet, idx) => (
  ...
  <div className="h-full w-full flex flex-col p-10 gap-6 relative overflow-hidden bg-[#0a0a0f]" style={{ width: '1280px', height: '720px' }}>
    ...
    <div className="flex-1 grid grid-cols-3 gap-5 items-stretch z-10 pb-4">
      {(slide.tiers || []).map((tier, idx) => (
        <div
          key={idx}
          className={`relative group flex flex-col gap-4 p-6 rounded-2xl border transition-all duration-500 ${
            idx === 0 
        ...
        >
          ...
          <div className="space-y-1">
            <h3 className="text-xl font-black text-white tracking-tight leading-none uppercase">{tier.name}</h3>
            <p className="text-base font-bold text-primary">{tier.price}</p>
          </div>
...
const CtaSlide = ({ slide }: { slide: Slide }) => (
  <div className="h-full w-full flex flex-col justify-center items-center p-12 gap-8 text-center relative overflow-hidden bg-[#0a0a0f]" style={{ width: '1280px', height: '720px' }}>
    ...
    <div className="z-10 space-y-6 max-w-3xl">
      <h2 className="text-5xl md:text-6xl font-black tracking-tighter text-white leading-none uppercase italic drop-shadow-2xl">
        {slide.title}
      </h2>
      ...
      {slide.cta && (
        <div className="inline-flex items-center gap-3 mt-4 px-10 py-4 rounded-full bg-primary text-white font-black text-lg shadow-[0_10px_20px_rgba(var(--primary),0.3)] hover:scale-105 transition-transform cursor-pointer uppercase tracking-wider">
          {slide.cta}
    ...
    <div className="absolute bottom-10 z-10">
      <div className="flex flex-col items-center gap-2">
        ...
        <p className="text-[9px] uppercase tracking-[0.4em] text-white/20 font-black">Future Events Ecosystem</p>
      </div>
    </div>
  </div>
);
...
const AIPitchDeck = ({ data, eventId }: AIPitchDeckProps) => {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Hidden PDF slides container - Optimized for high-quality capture */}
      <div 
        aria-hidden="true"
        style={{ 
          position: 'fixed',
        ...
        {data.slides.map((s, i) => (
          <div
            ...
            style={{
              ...
              position: 'relative',
              overflow: 'hidden',
              // Use standard system font stack for maximum compatibility
